...
int Dimension::getOriginalIndex() const { return originalIndex; }
...
// gets the data calibrated, but not inverted or shifted data for the set of the
// passed index(dataIndex)
...
// gets the name
string* Dimension::getName() { return &name; }

// sets the name and returns the old name
void Dimension::setName(string* newName) { name = *newName; }

// sets the data of the set at the passed index(dataIndex) to the passed
// value(newData), alters the original data
...
// multiplies all the data in the dimension by the passed double, does not alter
// original data
...
// divides all the data in the dimension by the passed double, does not alter
// original data
...
// adds the passed double to all the data in the dimension, does not alter
// original data
...
double Dimension::getShift() { return shiftAmount; }

bool Dimension::isInverted() { return inverted; }

// toggles whether the data is inverted
void Dimension::invert() { inverted = !inverted; }

// gets the number of sets in the dimensions
int Dimension::size() const { return data.size(); }

// returns whether there is artificial calibration or not
bool Dimension::isArtificiallyCalibrated() { return useArtificialCalibration; }
...
double Dimension::getArtificialMaximum() const { return artificialMaximum; }

// gets the artficial calibration maximum for the dimension
double Dimension::getArtificialMinimum() const { return artificialMinimum; }
