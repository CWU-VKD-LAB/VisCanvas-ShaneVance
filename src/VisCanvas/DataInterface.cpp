...
bool DataInterface::readFile(string* filePath) {
  ...
  if (isCustomFileFormat =
          ((*(*fileLines)[0])[0] == "VisCanvas Save File Format")) {
...
bool DataInterface::saveToFile(std::string* filePath) {
  ...
  for (int i = 1; i < getClassAmount(); i++) {
    ...
    saveFile << (*color)[0] << "," << (*color)[1] << "," << (*color)[2] << ","
             << (*color)[3];
    saveFile << "\n";
  }
  saveFile << "\n\n";
  ...
  // make the artificial calibration exists
  saveFile << "class Color," << *(getClassName(i)) << "," << i << "," << red <<
  "," << green << "," << blue << "," << alpha << "\n";
  }
  */
  ...
  for (int i = 0; i < getClusterAmount(); i++) {
    ...
    saveFile << clusters[i].getColor()->at(0) << ",";
    saveFile << clusters[i].getColor()->at(1) << ",";
    saveFile << clusters[i].getColor()->at(2) << ",";
    saveFile << clusters[i].getColor()->at(3) << ",";
    saveFile << clusters[i].getAlphaTwo() << "\n";
  }
  ...
  std::vector<double>* componentsBackgroundColor =
      backgroundColor.getColorComponents();
  ...
  std::vector<double>* componentsSelectedSetColor =
      selectedSetColor.getColorComponents();
  ...
  /*
  // put read parameters for basic files in
  saveFile << "Read Set Names," << isReadSetNames() << "\n";
  saveFile << "Read Dimension Names," << isReadDimensionNames() << "\n";
  saveFile << "Read Class Names," << isReadClassNames() << "\n";
  */
...
bool DataInterface::isReadSetNames() { return readSetNamesBasic; }

// returns whether dimension names will be read from basic files
bool DataInterface::isReadDimensionNames() { return readDimensionNamesBasic; }

// returns whether class names will be read from basic files
bool DataInterface::isReadClassNames() { return readClassNamesBasic; }

// gets the number of classes in the data
int DataInterface::getClassAmount() const { return dataClasses.size(); }
...
int DataInterface::getDimensionAmount() const { return dataDimensions.size(); }
...
// gets the original data in the set of the passed index(setIndex), for the
// passed dimension(indexOfData)
...
// sets the data in the set of the passed index(setIndex), for the passed
// dimension(indexOfData), to the passed value(newDataValue)
double DataInterface::setData(int setIndex, int indexOfData,
                              double newDataValue) {
  // check if indexes are in bounds
  if (setIndex >= getSetAmount() || setIndex < 0) {
    return 0.0;
...
/*
moves the position of a dimensions from the passed index(indexOfDimension) to at
the other passed index(indexOfInsertion) Note: the new position is calculated
before the dimension being moved is removed from its place so if the data had
dimensions 0,1,2,3 and the call moveData(1, 3) was made the dimensions would
become 0,2,1,3 not 0,2,3,1
*/
bool DataInterface::moveData(int indexOfDimension, int indexOfInsertion) {
  ...
  if (indexOfInsertion >= getDimensionAmount() ||
      indexOfDimension >= getDimensionAmount()) {
    return false;
  }
...
// gets the name of the class at the passed index(classIndex) to the passed
// string(newName)
...
// gets whether the calibration for the dimension(dimensionIndex) is artificial
// or relative
...
// sets the calibration to use the data's(not the artificial) maximum and
// minimum in dimension at the passed index(dimensionIndex)
...
void DataInterface::setCalibrationBounds(int dimensionIndex, double newMaximum,
                                         double newMinimum) {
  if (dimensionIndex >= dataDimensions.size() || dimensionIndex < 0) {
    return;
  }
...
// gets the artificial maximum for the dimension at the passed
// index(dimensionIndex)
...
// gets the artificial minimum for the dimension at the passed
// index(dimensionIndex)
...
/*
// sets the calibration to use the data's(not the artificial) maximum and
minimum in dimension at the passed index(dimensionIndex), of the dimension at
the passed index(dimensionIndex) void
DataInterface::clearArtificialCalibration(int dimensionIndex) { if
(dimensionIndex >= dataDimensions.size() || dimensionIndex < 0) { return;
}
return dataDimensions[dimensionIndex]->clearArtificialCalibration();
}
// sets the bounds to be used for artificial calibration at the passed
index(dimensionIndex) void DataInterface::setCalibrationBounds(int
dimensionIndex, double newMaximum, double newMinimum) { if (dimensionIndex >=
dataDimensions.size() || dimensionIndex < 0) { return;
}
return dataDimensions[dimensionIndex]->setCalibrationBounds(newMaximum,
newMinimum);
}
// gets the artificial maximum for the dimension at the passed
index(dimensionIndex) double DataInterface::getArtificialMaximum(int
dimensionIndex) const { if (dimensionIndex >= dataDimensions.size() ||
dimensionIndex < 0) { return 1.0;
}
return dataDimensions[dimensionIndex]->getArtificialMaximum();
}
// gets the artificial minimum for the dimension at the passed
index(dimensionIndex) double DataInterface::getArtificialMinimum(int
dimensionIndex) const { if (dimensionIndex >= dataDimensions.size() ||
dimensionIndex < 0) { return 0.0;
}
return dataDimensions[dimensionIndex]->getArtificialMinimum();
}
*/
// add the passed double(amountToAdd) to all the data in the dimension at the
// index(dimensionIndex)
...
// sets the name of the class at the passed index to the passed string and
// returns the old name full implementation after file parsing
...
void DataInterface::addClass() {
  dataClasses.push_back(DataClass(
      dataClasses.size(), "Class " + std::to_string(dataClasses.size())));
}
...
// sets the name of the class at the passed index(classIndex) to the passed
// string(newName)
...
Gets the number of sets in the class at the passed index(classIndex)
*/
void DataInterface::setClassColor(int classIndex,
...
// sets the name of the set at the passed index to the passed string and returns
// the old name full implementation after file parsing
...
std::vector<double>* DataInterface::getSetColor(int setIndex) {
  ...
  std::vector<double>* color =
      dataClasses[dataSets[setIndex].getClass()].getColor();
...
// full implementation after file parsing
std::string* DataInterface::getXAxisName() { return nullptr; }

// gets the name of the y-axis
// full implementation after file parsing
string* DataInterface::getYAxisName() { return nullptr; }

// gets the name of the maximum of the y axis
// full implementation after file parsing
string* DataInterface::getYMaxName() { return nullptr; }

// gets the name of the minimum of the y axis
// full implementation after file parsing
string* DataInterface::getYMinName() { return nullptr; }

// sorts the dimensions in ascending order by the data corresponding to the
// passed set index
...
// sorts the dimensions in descending order by the data corresponding to the
// passed set index(setIndex)
...
int DataInterface::getNoteAmount() { return notes.size(); }
...
// compares the data of a each set to the set at the passed index and checks if
// the data is within the radius of the data of the passed set if the passed
// bool(meanMedian) is true the mean will be used and otherwise the median will
// be
void DataInterface::hypercube(int setIndex, double radius) {
  ...
  for (int i = 0; i < getSetAmount(); i++) {
    ...
    if (i == setIndex) {
      // do nothing just don't bother checking if the set is within the radius
      // because it is
    } else {
  ...
  std::string clusterName =
      *(dataSets[setIndex].getName()) + " " + std::to_string(radius);
...
bool DataInterface::isUseMeanForClusters() { return useMean; }

// returns whether the clusters will use mean or median
bool DataInterface::getUseMeanForClusters() { return useMean; }
...
double DataInterface::getRadius() { return radius; }

void DataInterface::setRadius(double newRadius) { radius = newRadius; }
...
int DataInterface::getSelectedSetIndex() {
  ...
  return (dataClasses[selectedClassIndex].getSetsInClass())
      ->at(selectedSetIndex);
}

// increases the index of the selected class to the passed
// index(newSelectedClassIndex)
void DataInterface::incrementSelectedClassIndex() {
  ...
  // ensure class has sets
  while (dataClasses[newSelectedClassIndex].getSetNumber() == 0 &&
         numberOfClassesChecked <= getClassAmount()) {
...
// decreases the index of the selected class to the passed
// index(newSelectedClassIndex)
...
int DataInterface::getSelectedClassIndex() const { return selectedClassIndex; }

// sets the selected set's color to the colors in the passed double
// array(newSelectedSetColor)
void DataInterface::setSelectedSetColor(
    std::vector<double>& newSelectedSetColor) {
  if (newSelectedSetColor.size() < 4) {
    return;
  }
...
bool DataInterface::isPaintClusters() const { return paintClusters; }
...
int DataInterface::getClusterAmount() { return clusters.size(); }

// the minimum value for the cluster data
double DataInterface::getClusterMinimum(int clusterIndex,
                                        int dimensionIndex) const {
  if (dimensionIndex >= getDimensionAmount() || dimensionIndex < 0) {
    return 0.0;
  }
  return clusters[clusterIndex].getMinimum(dimensionIndex) +
         dataDimensions[dimensionIndex]->getShift();
}

// the mean value for the cluster data
double DataInterface::getClusterMiddle(int clusterIndex,
                                       int dimensionIndex) const {
  if (dimensionIndex >= getDimensionAmount() || dimensionIndex < 0) {
    return 0.0;
  }
  return clusters[clusterIndex].getMiddle(dimensionIndex) +
         dataDimensions[dimensionIndex]->getShift();
}

// the maximum value for the cluster data
double DataInterface::getClusterMaximum(int clusterIndex,
                                        int dimensionIndex) const {
  if (dimensionIndex >= getDimensionAmount() || dimensionIndex < 0) {
    return 0.0;
  }
  return clusters[clusterIndex].getMaximum(dimensionIndex) +
         dataDimensions[dimensionIndex]->getShift();
}
...
void DataInterface::setClusterColor(int clusterIndex,
                                    std::vector<double>* newColor) {
  if (clusterIndex < 0 || clusterIndex >= getClusterAmount()) {
    return;
  }
...
// gets the second alpha of the cluster
double DataInterface::getAlphaTwo(int clusterIndex) {
  if (clusterIndex < 0 || clusterIndex >= getClusterAmount()) {
    return 0.0;
  }
  return clusters[clusterIndex].getAlphaTwo();
}

// sets the second alpha of the cluster
void DataInterface::setAlphaTwo(int clusterIndex, double newAlphaTwo) {
  if (clusterIndex < 0 || clusterIndex >= getClusterAmount()) {
    return;
  }
  clusters[clusterIndex].setAlphaTwo(newAlphaTwo);
}
...
std::vector<std::vector<std::string>*>* DataInterface::readFileIntoVector(
    std::string* fileName) {
...
std::vector<std::string>* DataInterface::tokenizeString(
    std::string* stringToTokenize) {
  ...
  while (startToken < stringToTokenize->size()) {
    ...
    std::string currentToken =
        (stringToTokenize->substr(startToken, lengthToken));
...
bool DataInterface::readBasicFile(
  ...
  try {
    ...
    if (readClassNamesBasic) {
      for (int i = startRow; i <= lastDataLine; i++) {
        std::string setClassName =
            (*(*fileContents)[i])[(*fileContents)[i]->size() - 1];
        bool newClass = true;
...
// reads the contents of the file, at fileName, into a vector
void DataInterface::readCustomFile(
    std::vector<std::vector<std::string>*>* fileContents) {
  // ensure file has data
  if (fileContents->size() < 2) {
    return;
  ...
  std::vector<std::string> headers =
      std::vector<std::string>((*fileContents)[0]->size() - 2);
...
void DataInterface::parseLine(std::vector<std::string>* lineTokens) {
  ...
  // read data
    ...
    if (lineTokens->size() >= 8) {
      ...
      std::vector<double> newColor = std::vector<double>();
      newColor.push_back(stod((*lineTokens)[3]));
      newColor.push_back(stod((*lineTokens)[4]));
      newColor.push_back(stod((*lineTokens)[5]));
      newColor.push_back(stod((*lineTokens)[6]));
      double newAlphaTwo = stod((*lineTokens)[7]);
      if (index < 0 || index >= this->getSetAmount()) {
        return;
      } else {
        if (radius < 0) {
        ...
        this->clusters[this->clusters.size() - 1].setColor(&newColor);
        this->clusters[this->clusters.size() - 1].setAlphaTwo(newAlphaTwo);
      }
    }
...
// an ascending(left to right) merge sort of the passed dimension list by the
// set at the passed index
std::list<Dimension*>* DataInterface::mergeSortAscending(
    std::list<Dimension*>* listToSort, int setIndex) {
...
// an descending(left to right) merge sort of the passed dimension list by the
// set at the passed index
std::list<Dimension*>* DataInterface::mergeSortDescending(
    std::list<Dimension*>* listToSort, int setIndex) {
  if (listToSort->size() <= 1) {
    return listToSort;
  }
...
std::list<Dimension*>* DataInterface::mergeSortOriginal(
    std::list<Dimension*>* listToSort) {
  if (listToSort->size() <= 1) {
    return listToSort;
  }
