...
double DataNode::getData() const { return currentData; }

// gets the data that the node was originally set to hold
double DataNode::getOriginalData() const { return originalData; }
...
void DataNode::resetData() { currentData = originalData; }

// multiplies the data held by the node, by the passed value(multiplier)
void DataNode::multiplyData(double multiplier) { currentData *= multiplier; }

// divides the data held by the node, by the passed value(divisor)
void DataNode::divideData(double divisor) { currentData /= divisor; }

// add the passed value(addend) to the data held by the node
void DataNode::addToData(double addend) { currentData += addend; }
