...
SetCluster::SetCluster(ColorCustom& clusterColor,
                       std::vector<int>* newSetsInCluster) {
  color = clusterColor;
  color.setAlpha(0.5);
  setsInCluster = std::vector<int>();
  minimumValues = std::vector<double>();
...
// creates the cluster with the passed sets and values from the passed
// dimensions
SetCluster::SetCluster(ColorCustom& clusterColor,
                       std::vector<int>* newSetsInCluster,
                       std::vector<Dimension*>* dimensionsToCalculateWith) {
  // intialize fields
  color = clusterColor;
  color.setAlpha(0.5);
  setsInCluster = std::vector<int>();
...
// gets the calculates the minimum value of dimension for the sets whose indexes
// are passed(setIndexes)
double SetCluster::getMinimumValue(Dimension* dimension,
                                   std::vector<int>* setIndexes) {
  double minimum = 0.0;
  std::sort(setIndexes->begin(), setIndexes->end());
  // check if the dimension has enough sets for the sets in the passed
  // vector(setIndexes)
...
// gets the calculates the mean value of dimension for the sets whose indexes
// are passed(setIndexes)
double SetCluster::getMeanValue(Dimension* dimension,
                                std::vector<int>* setIndexes) {
  double sum = 0.0;
  std::sort(setIndexes->begin(), setIndexes->end());
  // check if the dimension has enough sets for the sets in the passed
...
// gets the calculates the median value of dimension for the sets whose indexes
// are passed(setIndexes)
double SetCluster::getMedianValue(Dimension* dimension,
                                  std::vector<int>* setIndexes) {
  double sum = 0.0;
  std::sort(setIndexes->begin(), setIndexes->end());
  // check if the dimension has enough sets for the sets in the passed
  ...
  for (int i = 0; i < setIndexes->size(); i++) {
    ...
    values.push_back(dimension->getCalibratedData(currentIndex));
  }
...
// gets the calculates the maximum value of dimension for the sets whose indexes
// are passed(setIndexes)
double SetCluster::getMaximumValue(Dimension* dimension,
                                   std::vector<int>* setIndexes) {
  double maximum = 0.0;
  std::sort(setIndexes->begin(), setIndexes->end());
  // check if the dimension has enough sets for the sets in the passed
...
// returns true if the set is added and false if the set was already in the
// cluster
...
int SetCluster::getSetNumber() { return setsInCluster.size(); }

// gets the index of the set in this cluster for the set at the passed
// index(clusterRelativeIndex), which is the index the set index is stored at
int SetCluster::getIndexOfSet(int clusterRelativeIndex) {
  if (clusterRelativeIndex >= setsInCluster.size() ||
      clusterRelativeIndex < 0) {
    return -1;
  }
...
void SetCluster::calculateValues(
    std::vector<Dimension*>* dimensionsToCalculateWith) {
  // get the values from the dimensions
  // check if there are any dimensions passed
  ...
  // check if then largest set index is is within the dimensions passed
  else if (setsInCluster[setsInCluster.size() - 1] >=
           (*dimensionsToCalculateWith)[0]->size()) {
    // do nothing dimensions cannot be used for this cluster
  } else {
    minimumValues.clear();
    meanValues.clear();
    ...
    for (int i = 0; i < dimensionsToCalculateWith->size(); i++) {
      double min =
          getMinimumValue((*dimensionsToCalculateWith)[i], &setsInCluster);
      double mean =
          getMeanValue((*dimensionsToCalculateWith)[i], &setsInCluster);
      double median =
          getMedianValue((*dimensionsToCalculateWith)[i], &setsInCluster);
      double max =
          getMaximumValue((*dimensionsToCalculateWith)[i], &setsInCluster);
...
void SetCluster::invertValues(int dimensionToInvertValuesAt) {
  if (dimensionToInvertValuesAt >= minimumValues.size() ||
      dimensionToInvertValuesAt < 0) {
    return;
  }
  minimumValues[dimensionToInvertValuesAt] =
      1 - minimumValues[dimensionToInvertValuesAt];
  meanValues[dimensionToInvertValuesAt] =
      1 - meanValues[dimensionToInvertValuesAt];
  medianValues[dimensionToInvertValuesAt] =
      1 - medianValues[dimensionToInvertValuesAt];
  maximumValues[dimensionToInvertValuesAt] =
      1 - maximumValues[dimensionToInvertValuesAt];
}

// move the position of the values in the set(at originalIndex) to the index
// after indexBeforeNewIndex
bool SetCluster::moveValues(int originalIndex, int indexOfInsertion) {
  ...
  if (indexOfInsertion >= minimumValues.size() ||
      originalIndex >= minimumValues.size()) {
    return false;
  }
...
// gets the second alpha
double SetCluster::getAlphaTwo() { return alphaTwo; }

// sets the second alpha
void SetCluster::setAlphaTwo(double newAlphaTwo) { alphaTwo = newAlphaTwo; }
...
double SetCluster::getRadius() { return radius; }
...
int SetCluster::getOriginalSet() { return originalSet; }
...
std::string* SetCluster::getName() { return &name; }

// sets the name of this cluster
void SetCluster::setName(std::string* newName) { name = (*newName); }

// gets the sets in the cluster
std::vector<int>* SetCluster::getSets() { return &setsInCluster; }

// get whether the the cluster uses the mean or median
bool SetCluster::isUseMean() { return useMean; }

// set whether the the cluster uses the mean or median
void SetCluster::setUseMean(bool newUseMean) { useMean = newUseMean; }
