...
class DataInterface {
 public:
  ...
  // gets the data in the set of the passed index(setIndex), for the passed
  // dimension(indexOfData)
  ...
  // moves the dimension at the passed index(indexOfDimension) to the spot after
  // the other index(indexBeforeInsertion)
  ...
  // gets whether the calibration for the dimension(dimensionIndex) is
  // artificial or relative
  ...
  void setCalibrationBounds(int dimensionIndex, double newMaximum,
                            double newMinimum);
  ...
  */
  ...
  // sets the color for the class at the passed class index(classIndex) to the
  // passed color(newColor)
  void setClassColor(int classIndex, std::vector<double>* newColor);

  // gets the name of the set in the class at the passed index(classIndex) at
  // the passed class relative index(setIndex)
  ...
  // sorts the data by the set at the passed index, in ascending order(left to
  // right)
  void sortAscending(int setIndex);
  // sorts the data by the set at the passed index, in descending order(left to
  // right)
  void sortDescending(int setIndex);
  // places the dimensions back in the original order
  void sortOriginal();

  /* adjusts the data of each dimension by shifting the data of each dimension
  by the amount necessary to put the dat of the set at the passed
  index(setIndex) to the passed value(levelValue)*/
  ...
  // calibrates each dimension's data to the [0,1] space, dimensions calibrated
  // individually
  void calibrateData();
  ...
  // compares the data of a each set to the set at the passed index and checks
  // if the data is within the radius of the data of the passed set
  ...
  double getAlphaTwo(int clusterIndex);
  // gets the second alpha of the cluster
  void setAlphaTwo(int clusterIndex, double newAlphaTwo);
 ...
 private:
  ...
  std::vector<std::vector<std::string>*>* readFileIntoVector(
      std::string* fileName);
  ...
  // counts the number of characters in the passed string(line) that are one of
  // the characters in the passed vector(characters)
  ...
  std::list<Dimension*>* mergeSortAscending(std::list<Dimension*>* listToSort,
                                            int setIndex);

  // an descending(left to right) merge sort of the passed dimension list by the
  // set at the passed index
  std::list<Dimension*>* mergeSortDescending(std::list<Dimension*>* listToSort,
                                             int setIndex);
