...
void DataInterface::parseLine(std::vector<std::string>* lineTokens) {
	...
	/*
	else if ((*lineTokens)[0].compare("Read Set Names") == 0) {
	if (lineTokens->size() >= 2) {
	bool boolIsReadSetNames = stoi((*lineTokens)[1]);
	setReadSetNames(boolIsReadSetNames);
	}
	}
	else if ((*lineTokens)[0].compare("Read Dimension Names") == 0) {
	if (lineTokens->size() >= 2) {
	bool boolIsReadDimensionNames = stoi((*lineTokens)[1]);
	setReadDimensionNames(boolIsReadDimensionNames);
	}
	}
	else if ((*lineTokens)[0].compare("Read Class Names") == 0) {
	if (lineTokens->size() >= 2) {
	bool boolIsReadClassNames = stoi((*lineTokens)[1]);
	setReadClassNames(boolIsReadClassNames);
	}
	}
	*/
	/*
